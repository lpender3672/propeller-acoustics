...
\begin{figure}[H]
    ...
    \begin{tikzpicture}
        ...
        \begin{scope}[rotate=225, xshift=-2cm, yshift=-1cm]
...
\begin{equation}
    p(t) = \sum_{m=-\infty}^\infty P_{mB} \exp(-jmB\Omega t) + \sum_{m=-\infty}^\infty P_{mB} \exp(-jmB(\Omega t + \varepsilon))
    \label{eq:Dobrzynski}
\end{equation}

However, the next section explores this more generally.
...
\end{equation}
To validate this a small, constant sweep offset $\varepsilon$ is applied to the otherwise identical second blade.
\begin{equation}
    MCA_2 = MCA_1 + r_0 \varepsilon
\end{equation}
When substituted into equation \ref{eq:final_integral} the noise is found to be
\begin{equation}
    P^{(2)}_{mB} = P^{(1)}_{mB} \exp ( j mB \varepsilon )
\end{equation}
And the result by Dobrzynski is recovered (Equation \ref{eq:Dobrzynski}).


\section{ $\boldsymbol{\xi_0}$ Noncompactness }

Superimposing sources in $\xi_0$ is only valid if the distance between them are small relative to the acoustic wavelength.

We rewrite $\psi$ as
\begin{equation}
    \psi \left(\frac{\omega}{U}, r_0, \frac{\varpi}{U}\right) = \int_{-\infty}^\infty \int_{-\infty}^\infty g(\gamma_0, r_0, \xi_0) \exp \left(j \frac{\omega}{U} \gamma_0 \right) d\gamma_0 \exp \left(j \frac{\varpi}{U} \xi_0 \right) d\xi_0
    \label{eq:doublefourier}
\end{equation}

\begin{equation}
    g(\gamma_0, r_0, \xi_0) = \frac{1}{4\pi^2 U^2} \int_{-\infty}^\infty \int_{-\infty}^\infty \psi\left(\frac{\omega}{U}, r_0, \xi_0\right) \exp \left(-j \frac{\omega}{U} \gamma_0 \right) d\omega \exp \left(-j \frac{\varpi}{U} \xi_0 \right) d\varpi
    \label{eq:inverse_doublefourier}
\end{equation}

Then substituting this into equation \ref{eq:sifted_progress}
...
\section{Introduction of Hansons Theory for final report}
This section forms a plan of the introduction of hansons helicoidal coordinates to the theory.
A necessary complication is required to properly understand Hansons theory which involves the Introduction of helicoidal coordinates.

It becomes necessary to explain the origin of harmonics in the pressure field.

The sound waveform for $B$ propeller blades is the sum of the contributions from each blade.
However, the signal from each blade experiences a phase-lag 
\begin{equation}
    p = \sum_{m=0}^{B-1} s \Bigl(
      t
      - \underbrace{\frac{m}{f_0}}_{\substack{\text{“time”}\\\text{phase-lag}}}
      - \underbrace{\frac{r_m(t)}{c}}_{\substack{\text{“retarded-time”}\\\text{phase-lag}}}
    \Bigr)
\end{equation}

Where $r_m$ is the distance from the observer to the mth blade.
The position of blade tip and observer are defined as follows.
\begin{align}
    \mathbf x_m(t) &= r_t \left(\cos(\Omega t + 2\pi m/B),\;\sin(\Omega t + 2\pi m/B),\;0\right) \\
    \mathbf r &= r\bigl(\sin\theta,\,0,\,\cos\theta\bigr)
\end{align}
Then, assuming the observer is in the far field $r >> r_t$
\begin{align}
    r_m(t) &= \bigl\lvert \mathbf r - \mathbf x_m \bigr\rvert
    \approx r
    -\hat{\mathbf r}\cdot\mathbf x \\
    r_m(t) &\approx r - r_t \sin\theta\;\cos\!\bigl(\Omega t + 2\pi m/B\bigr)
\end{align}

\begin{equation}
    P(\omega) \sum_{m=0}^{B-1} \exp\!\left[-j\omega \left( \frac{m}{f_0} + \frac{r - r_t \sin\theta\cos(\Omega t + 2\pi m/B)}{c_0} \right)\right]
\end{equation}

\begin{equation}
    =P(\omega) \exp \left(-j\omega \frac{r}{c_0} \right)
  \sum_{n=-\infty}^\infty(-j)^nJ_n(kR\sin\theta)\,e^{-jn\Omega t}
  \sum_{m=0}^{B-1}e^{-j\frac{2\pi m}{B}\bigl(n+\omega/\Omega\bigr)}
\end{equation}

\begin{equation}
    \propto\sum_{n}\!J_n(kR\sin\theta)e^{-jn\Omega t}\,
    \bigl[\sum_{m=0}^{B-1}e^{-j2\pi m(n+\omega/\Omega)/B}\bigr]
\end{equation}


% insert figure of helicoidal coordinates
