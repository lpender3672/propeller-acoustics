...
The quadrupole is the stress tensor $T_{ij}$ which is neglected from here on WHY?.
...
\end{equation}
Where $R = |x - y|$ is the distance between the source and observer points.
...
Then $ dS = d\gamma_0 dr_0 $ neglecting the 2D coordinate transformation Jacobian which is reduced to 0?!.

This then turns equation \ref{eq:goldstein} into
\begin{equation}
p'(x, t) = -\int_{-\infty}^\infty \int_0^T \int_{-\infty}^\infty \left( \rho_0 V_n \frac{\partial G}{\partial \tau} + f_i \frac{\partial G}{\partial \gamma_i} \right)d\gamma_0 dr_0 d\tau
\end{equation}
...
\begin{equation}
    p' = \iiiint G \left(\frac{\partial Q}{\partial \tau} + \frac{\partial R_i}{\partial \boldsymbol{\gamma}_i} \right) d\tau d\xi_0 d\gamma_0 dr_0 - \iiint \Big[ QG \Big]_{-T}^{+T} dV(\tau) - \int_{-T}^{+T} \iint R_i G n_i dS(\tau) d\tau
\end{equation}
...
For B blades, the mih harmonic of blade passing
...
\begin{equation}
    \psi_V\left( \frac{mB\Omega}{U}, r_0, \xi_0 \right) = \rho_0 U^2 \int \frac{\partial^2 }{\partial \eta^2} \bigg[ h(\eta, r_0) \delta(FA + \xi_0) \bigg] \exp\left( j \frac{mB\Omega}{U} \eta \right) d\eta
\end{equation}
Integration by parts twice gives

\begin{equation}
    \psi_V\left( \frac{mB\Omega}{U}, r_0, \xi_0 \right) = - \rho_0 U^2 k_x^2 t_b \delta(FA + \xi_0) \exp\left( j k_x \frac{MCA}{b} \right) \Psi_V(k_x)
\end{equation}
Where
\begin{equation}
...
\begin{equation}
    k_x = \frac{2mB B_D M_T}{M_r}
\end{equation}
Similarly for drag
\begin{equation}
    \psi_D\left( \frac{mB\Omega}{U}, r_0, \xi_0 \right) = \rho_0 U^2 \int \frac{\partial }{\partial \gamma_0} \Big[ \frac{1}{2}\rho_0U^2 C_D f_D\left(X-\frac{MCA}{b}\right) \delta(FA + \xi_0) \Big] \exp\left( j \frac{mB\Omega}{U} \gamma_0 \right) d\gamma_0
\end{equation}
And again integration by parts gives
\begin{equation}
    \psi_D\left( \frac{mB\Omega}{U}, r_0, \xi_0 \right) = \frac{1}{2} \rho_0 U^2 K_x C_D \int f_L(X) \exp(jk_x X) dX
\end{equation}
Lift is slightly different as $f_2$ is in the $\xi_0$ direction instead of $\gamma_0$.
\begin{equation}
    \psi_D\left( \frac{mB\Omega}{U}, r_0, \xi_0 \right) = \rho_0 U^2 \int \frac{\partial }{\partial \xi_0} \Big[ \frac{1}{2}\rho_0U^2 C_L f_L\left(X-\frac{MCA}{b}\right) \delta(FA + \xi_0) \Big] \exp\left( j \frac{mB\Omega}{U} \xi_0 \right) d\xi_0
\end{equation}
