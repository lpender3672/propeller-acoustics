...
\begin{itemize}
    ...
    \item March 12th Encoder communication broken, thought ODrive was fried. The 1> MHz clock pin was found to only work with the additional capacitance of an oscilloscope probe. Soldered on a 200pF capacitor to the pin and now it works.
...
\section{Speed and distance invariance}

\begin{equation}
    \text{SPL} \propto \frac{\rho (\omega r)^2}{R/r} f(\theta, n, \text{geometry}) 
\end{equation}

Recordings were then made at different speeds, distances and angles to the propeller.

\begin{equation}
    \log_{10}(\text{SPL}) = \alpha \log_{10}(\omega) + \beta \log_{10}(r/R) + \log_{10}(f(\theta, n, \text{geometry}))
\end{equation}

The coefficients of 2 and -1 will now be verified by experimental data.
