...
def calc_std_forces(aero_data, tcal_data, qcal_data):
    ...
    thrust_samples_cal = np.stack([
        np.interp(raw_thrust[i, :], tcal_data[:, 0, 0], tcal_data[:, 1, 0], np.NaN, np.NaN)
        for i in range(raw_thrust.shape[0])
    ], axis=0)
    torque_samples_cal = np.stack([
        np.interp(raw_torque[i, :], qcal_data[:, 0, 1], qcal_data[:, 1, 1], np.NaN, np.NaN)
...
def plot_prop(prop_result_path, ax1, ax2, cal_data, label=None,
              tcal_data=None, qcal_data=None, rho=1.225):
    """
    ...
    mean_speed, mean_thrust, mean_torque = calc_mean_forces(aero_data, tcal_data, qcal_data)
    ...
    ax1.plot(speed_t, CT, '-o', markersize=3, label=label, zorder=1)
    ...
    ax2.plot(speed_q, CQ, '-o', markersize=3, label=label, zorder=1)
