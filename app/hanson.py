...
def hanson(
    oper: dict, prop: dict, obs: dict, ms: np.ndarray, obsmove: bool = False
) -> np.ndarray:
    """
    ...
    for o in range(Nobs):
        ...
        for i, m in enumerate(ms):
            ...
            ) / ((8 * np.pi * y / (2 * prop["rt"]) * dopfac_o) + 1e-10)
...
def hanson_secondary_variables(av, compact_chord=True):
    ...
    yf = np.interp(
        np.linspace(0, 1, 2 * prop["nx"]), np.linspace(0, 1, n), av.airfoil_data[:, 1]
    )
    ...
    oper["Min"] = 0
    # % inflow Mach number [-]
    oper["Mfl"] = oper["V"] / oper["c0"]
    # % flight Mach number [-]
    oper["Mht"] = (
        np.sqrt((oper["Omega"] * 2 * np.pi * prop["rt"]) ** 2 + oper["V"] ** 2)
        / oper["c0"]
    )
    ...
    oper["Mr"] = np.sqrt(
        oper["Mx"] ** 2 + (oper["Mt"] * prop["r0_rt"]) ** 2
...
def hanson_av(av):
    ...
    vector_contributions = radial_noise_contributions(
        oper, prop, peak_observer, ms, False
    )
...
def validate():
    ...
    oper["Mfl"] = V / oper["c0"]
    # % flight Mach number [-]
    oper["Mht"] = np.sqrt((Omega * 2 * np.pi * prop["rt"]) ** 2 + V**2) / oper["c0"]
    # % Helical tip Mach number [-]
