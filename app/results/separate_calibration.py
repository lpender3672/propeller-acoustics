...
thrust_cal_data = np.load('app/results/thrust_cal.npy') # shape n, 2, 2 representing n samples, 0 for sensor data, 1 for applied force, 0 for thrust, 1 for torque
...
def plot_raw(axes):
    ...
    axes[0].plot(*sortx(mixed_cal_data[:,0,0], mixed_cal_data[:,1,0]), '-o', label='Mixed calibration')
    axes[1].plot(*sortx(mixed_cal_data[:,0,1], mixed_cal_data[:,1,1]), '-o', label='Mixed calibration')
...
def determine_coefficients(combined):
    ...
    # the torque is divided by a length to weight the torque and force equally in the least squares fit
...
def single_regression(thrust_cal_data, torque_cal_data):
    ...
    X = np.column_stack((S_N, np.ones_like(S_N))) # relation between torque sensor and thrust
    coeff_T, residuals_N, rank_N, s_N = np.linalg.lstsq(X, T, rcond=None)

    X = np.column_stack((S_T, np.ones_like(S_T))) # relation between thrust sensor and thrust
...
def plot_cal(axes):
    ...
    S_T = np.linspace(thrust_cal_data[:,0,0].min(), thrust_cal_data[:,0,0].max(), 100)
    S_N = np.linspace(thrust_cal_data[:,0,1].min(), thrust_cal_data[:,0,1].max(), 100)
    ...
    S_T = np.linspace(torque_cal_data[:,0,0].min(), torque_cal_data[:,0,0].max(), 100)
    S_N = np.linspace(torque_cal_data[:,0,1].min(), torque_cal_data[:,0,1].max(), 100)
