...
def fix_microphone_angles(mic_states_folder):
    ...
    for micf in mic_states_folder.glob("gantry*.csv"):
        ...
        micdata = np.loadtxt(micf, delimiter=",", dtype=str)
        theta1 = float(micdata[3,2])
        theta2 = float(micdata[4,2])
        theta3 = float(micdata[5,2])
        dist = float(micdata[4,3])
        ...
        if np.isclose(dist, 1270):
            ...
            micdata[3, 2] = str(new_theta1)
            micdata[5, 2] = str(new_theta3)
