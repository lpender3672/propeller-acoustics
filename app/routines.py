...
def foil_data(airfoil_data, alpha, Re, collect_cp=False):
    ...
    xf = XFoil()
    xf.airfoil = Airfoil(airfoil_data[:, 0], airfoil_data[:, 1])
    # xf.M = 0.0
    xf.max_iter = 100
    xf.verbose = False
...
def Viterna_extrapolation(xfoil_data, alpha, Re):
    ...
    b1 = CDmax
    a2 = (
        (CLstall - CDmax * np.sin(alpha_stall) * np.cos(alpha_stall))
        * np.sin(alpha_stall)
        / (np.cos(alpha_stall) ** 2)
    )
    ...
    if XFOIL_INSTALLED and xfoil_data.shape[1] >= 4:
        ...
        if isinstance(alpha, float):
            ...
            if alpha > np.max(alpha_data[Cl_valid]):
                ...
                Cl = np.interp(
                    alpha * 180 / np.pi, alpha_data[Cl_valid], Cl_data[Cl_valid]
                )
                Cd = np.interp(
                    alpha * 180 / np.pi, alpha_data[Cd_valid], Cd_data[Cd_valid]
                )
...
def calc_distribution(dist_index, ctrl_pts, x_dist):
    ...
    if dist_index == 0:
        ...
        y_dist = (
            coefficients[0] * x_dist**2 + coefficients[1] * x_dist + coefficients[2]
        )
    elif dist_index == 2:
        idx = np.argsort(x)
...
def load_prop_from_file(file_path, include_dists=False):
    ...
    distypes = ["linear", "quadratic", "spline", "custom", "inverse", "arctan"]
