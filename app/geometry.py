...
def rotate2(X, Z, theta):
    ...
    return (X * np.cos(theta) - Z * np.sin(theta)), (
...
def hermite_curve_and_tangents(P0, P1, T0, T1, num_points=50):
    ...
    for t in t_values:
        ...
        points.append(C_t)
        tangents.append(
            normalize(Ct_prime) if np.linalg.norm(Ct_prime) > 1e-12 else Ct_prime
        )
...
def boxes_intersect_3d(boxA, boxB):
    ...
    return not (
        (boxA[1] < boxB[0])
        or (boxA[0] > boxB[1])  # x-range
        or (boxA[3] < boxB[2])
        or (boxA[2] > boxB[3])  # y-range
        or (boxA[5] < boxB[4])
        or (boxA[4] > boxB[5])  # z-range
    )
...
def generate_blade_mesh(av, are_sections_tangent=True, apply_min_thickness=False):
    ...
    xf = np.interp(
        np.linspace(0, 1, 2 * av.prop["nx"]), np.linspace(0, 1, xnf.shape[0]), xnf
    )
    zf = np.interp(
        np.linspace(0, 1, 2 * av.prop["nx"]), np.linspace(0, 1, znf.shape[0]), znf
    )
    ...
    if av.prop["twinB"] == "Single":
        ...
        triangles = np.zeros(
            (2 * nf * (2 * Nsect + 1 + Ntip) + 2 * (nf - 2), 3), dtype=np.int64
        )
    ...
    if are_sections_tangent:
        ...
        if Ntip > 0:
            ...
            P0 = np.array(
                [*rotate2(0.0, radius[Nsect - 1], -sweep_angle[Nsect - 1]), 0]
            )
            P1 = np.array([*rotate2(0.0, radius[Nsect - 1], sweep_angle[Nsect - 1]), 0])

            T0 = -(
                np.array([X[start + 1, 0], Y[start + 1, 0], Z[start + 1, 0]])
                - np.array([X[start, 0], Y[start, 0], Z[start, 0]])
            )
            T1 = np.array(
                [
                    X[Nsect + 1, Nsect // 2],
                ...
                [X[Nsect, Nsect // 2], Y[Nsect, Nsect // 2], Z[Nsect, Nsect // 2]]
            )

        else:

            sx, Z[end, :] = rotate2(
                xf * chord[Nsect - 1], zf * chord[Nsect - 1], -twist[Nsect - 1]
            )
...
def generate_hub(
    ...
    inner_offset = (
        height_segments + 1
    ) * outer_segments + height_segments * radial_segments
    for i in range(radial_segments):
        for j in range(outer_segments_per_inner):
            k = (i * outer_segments_per_inner + j) % outer_segments
            next_k = (i * outer_segments_per_inner + j + 1) % outer_segments
...
def stitch_meshes(mesh1, mesh2):
    ...
    combined_vertices = np.vstack(
        (mesh1.vectors.reshape(-1, 3), mesh2.vectors.reshape(-1, 3))
    )
    unique_vertices, inverse_indices = np.unique(
        combined_vertices, axis=0, return_inverse=True
    )
...
def generate_tip_verticies(
    ...
    for i in range(len(points)):
        ...
        sized_airfoil = chord_factor * rotate_airfoil(
            transformed_airfoil, twist_values[i]
        )
