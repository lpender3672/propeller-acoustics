...
def generate_blade_mesh(av):
    ...
    sx, Z[0,:] = rotate(xf*chord[0], zf*chord[0], -twist[0])
    ...
    for i in range(1,Nsect+1):
        ...
        sx, sz = rotate(xf*chord[i-1], zf*chord[i-1], -twist[i-1])
    ...
    sx, sy = rotate(xf*chord[Nsect-1], zf*chord[Nsect-1], -twist[Nsect-1])
